...
description: Stop wasting hours on debugging a failed CI job, when you can go to the very remote machine that run your tests, open its state, and solve the issue.
keywords:
  - circleci
  - ci
...
Reliable issue reproduction is the absolute must in order to resolve any problem. We follow steps not only to cause an issue, but also to verify when it's been successfully fixed. There is a myriad of things that can affect an issue's reproduction: environment, connection speed, asynchronicity, dark magic. When your CI runs remotely, all of those factors are, inevitably, at play.
...
**Secure Shell** (also SSH) is a network protocol that encrypts data packages sent over an insecure connection.

> Briefly describe the principle of its work, add a diagram
...
Alternatively, check for any existing SSH keys on your machine by executing:

```bash
$ ls -la ~/.ssh
```

Running the command above will list the available SSH keys, if any. SSH keys file names usually look similar to:
...
In case you don't have any SSH keys, or would like to create a new one for the sake of this tutorial, please follow the next section.
...
> Please use your GitHub user email.

- `-t`, a type of key to create (`dsa`, `ecdsa`, `ed25519`, `rsa`);
...
Please follow the official instructions on [Adding a new SSH key to yout GitHub account](https://help.github.com/en/github/authenticating-to-github/adding-a-new-ssh-key-to-your-github-account). Once successful, you should see the SSH key in your [SSH and GPG keys](https://github.com/settings/keys) page.

---

## Running CircleCI job with SSH

Once our SSH setup is done, let's switch to the CircleCI part. In order to SSH into a remote host responsible for a particular job, that job must be run with SSH first. You can do that directly from the CircleCI UI by following these instructions:

1. Open the failed job detail.
